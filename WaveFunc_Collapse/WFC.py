...
def ReturnOnlyDuplicates(test):
    returnArr = np.full((test.size),None)
    size = 0
    for i in range(test.size):
        for j in range(test.size)[::-1]:
            if (i == j):
                ...
                returnArr[size] = test[i]
                size += 1
    returnArr = returnArr[0:size]
    
    return np.unique(returnArr)
def ShowOnScreen(x,y, input):
    ourFont = pygame.font.Font("freesansbold.ttf", 10)
    if (isinstance(input,np.ndarray) or isinstance(input, list)):
        text = ourFont.render((str(input)), True, (255,0,0))
    else : text = ourFont.render(str(input), True, (0,0,255))
    canvas.blit(text, (x * DIM_of_img, y * DIM_of_img))
...
def _setup():
    ...
    img1 = pygame.image.load("WaveFunc_Collapse\Demo_Tiles/blank.png")
    img2 = pygame.image.load("WaveFunc_Collapse\Demo_Tiles/up.png")
                        ...
                        for i in range(DIM_of_canvas):
                            ...
                            if (grid[index].collapsed):
                                ...
                                for k in range(len(tiles)):
                                    options[k] = k
                                validArray = np.full(0,int)
                                print(options)
                                #---look up                                
                                if(j > 0):
                                    up = grid[i + (j-1) * DIM_of_canvas]
                                    if up.collapsed == True : 
                                        collapsed += 1
                                        iscollapsed = True
                                        validOptions = np.full((len(tiles)), None)
                                        for option in up.options:
                                            valid = tiles[option].down
                                            validArray = np.concatenate((validArray, valid))
                                            validOptions = np.concatenate((validOptions, valid))
                                        _checkValid(options, validOptions)
                                        print("lookUp," + str(validArray))
                                ...
                                if(i < DIM_of_canvas -1):
                                    ...
                                    if right.collapsed == True : 
                                        ...
                                        for option in right.options:
                                            ...
                                            validOptions = np.concatenate((validOptions, valid))
                                        _checkValid(options, validOptions)
                                        print("lookRight," + str(validArray))
                                    
                                #---look down
                                if(j < DIM_of_canvas-1):
                                    down = grid[i + (j+1) * DIM_of_canvas]
                                    if down.collapsed == True : 
                                        collapsed += 1
                                        iscollapsed = True
                                        validOptions = np.full((len(tiles)), None)
                                        for option in down.options:
                                ...
                                if(i > 0):
                                    ...
                                    if left.collapsed == True : 
                                        ...
                                        for option in left.options:
                                            ...
                                            validOptions = np.concatenate((validOptions, valid))
                                        _checkValid(options, validOptions)
                                        print("lookleft," + str(validArray))
                                    

                                
                                nextGrid[index] = Cell_C()
                                if iscollapsed == False:
                                    returnable = np.full((len(tiles)), i)
                                    for k in range(len(tiles)):
                                        returnable[k] = k
                                elif (collapsed > 1):
                                    returnable = ReturnOnlyDuplicates(validArray)
                                else: returnable = valid


                                #ShowOnScreen(i,j,returnable)
                                nextGrid[index]._CellInit(returnable)
                                #print(options)
...
test2 = ReturnOnlyDuplicates(test)
print(test2)
